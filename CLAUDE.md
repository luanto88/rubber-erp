...
- Mỗi card: tên ngăn, loại NL, nguồn gốc/xử lý, chứng nhận, KL tươi/khô, thanh tiến độ ủ
...
// Bay hơi & Nitơ:
dat = max <= limit                          // luôn kiểm
     && (hasRange ? dr <= dr_limit : true)  // kiểm DR chỉ với TCCS/TCKH

// Po:
dat = min >= po_min
     && (hasRange && po_dr ? dr <= po_dr : true)

// PRI:
dat = mean >= pri_tb    // TB ≥ giới hạn TB
