-- Module KPI & 5S — Tự dọn các dòng kpi_monthly_scores "mồ côi" (tính bằng công thức RPC CŨ,
-- trước khi có renormalize ở 20260825_kpi_score_renormalize.sql) mỗi lần bấm "Tính điểm tháng".
-- Xem đầy đủ .claude/rules/27-kpi-module.md, mục "Kế hoạch phiên sau (2026-08-XX)" → Vấn đề 1.
--
-- Bug đã sửa: 20260825 chỉ INSERT/UPDATE các dòng nằm trong tập w_sum_eff > 0 — không có cơ chế
-- xóa dòng cũ của user giờ không còn dữ liệu nào (4 thành phần A/B/C/D đều thiếu). Các dòng đó
-- vẫn mang giá trị tính từ RPC cũ (COALESCE mặc định 100 mỗi thành phần thiếu → 100 × hệ số
-- chuyên cần sàn 0.75 = 75 điểm) và tồn tại vĩnh viễn cho tới khi bị dọn thủ công.
--
-- Quyết định đã chốt với người dùng: RPC tự dọn lặng lẽ mỗi lần tính điểm, không cần banner/cảnh
-- báo riêng cho việc dọn dẹp.
--
-- Giữ nguyên chữ ký hàm (3 tham số) — không cần DROP FUNCTION. Thân hàm copy nguyên vẹn logic
-- renormalize từ 20260825 (không đổi công thức), chỉ thêm 1 câu DELETE ngay sau khi tính xong
-- v_affected.
...
BEGIN
  ...
  -- Dọn các dòng "mồ côi" của tháng này — user không còn bất kỳ dữ liệu nào ở cả 4 thành phần
  -- (thường là kết quả của lần tính điểm bằng RPC CŨ trước khi có renormalize, hoặc dữ liệu nguồn
  -- đã bị xóa sau đó). Không đụng điểm đã khóa sổ.
  DELETE FROM public.kpi_monthly_scores ms
  WHERE ms.factory_id = p_factory_id
    AND ms.nam = p_nam
    AND ms.thang = p_thang
    AND ms.trang_thai <> 'da_khoa'
    AND NOT EXISTS (
      SELECT 1 FROM public.kpi_task_members tm
      JOIN public.kpi_tasks t ON t.id = tm.task_id
      WHERE tm.user_id = ms.user_id AND t.factory_id = p_factory_id AND tm.is_active = true
        AND t.ngay_giao BETWEEN v_month_start AND v_month_end
    )
    AND NOT EXISTS (
      SELECT 1 FROM public.kpi_5s_evaluations e
      WHERE e.nguoi_don_id = ms.user_id AND e.factory_id = p_factory_id
        AND e.tuan_bat_dau BETWEEN v_month_start AND v_month_end
    )
    AND NOT EXISTS (
      SELECT 1 FROM public.kpi_daily_evaluations de
      WHERE de.user_id = ms.user_id AND de.factory_id = p_factory_id
        AND de.ngay BETWEEN v_month_start AND v_month_end
    );
