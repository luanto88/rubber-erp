-- Fix regression: later inventory RPCs reintroduced ambiguous `document_id`
-- because RETURNS TABLE exposes an OUT param named `document_id`.
-- Use aliases for all reads from inventory_document_lines while preserving
-- shared oil stock logic from 20260511_shared_oil_stock_pools.sql.
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    WHERE idl.document_id = p_document_id
      AND idl.factory_id = p_factory_id
  ) THEN
  ...
  FOR v_line IN
    ...
    WHERE idl.document_id = p_document_id
      AND idl.factory_id = p_factory_id
    ORDER BY idl.created_at, idl.id
  LOOP
    SELECT * INTO v_item
    FROM inventory_items
    WHERE id = v_line.item_id AND factory_id = p_factory_id;
...
BEGIN
  ...
  LOOP
    ...
    IF COALESCE(v_item.uses_shared_oil_stock, false) THEN
      ...
      SELECT COALESCE(on_hand, 0) INTO v_current_stock
    ...
    IF COALESCE(NULLIF(trim(v_line.lot_no), ''), '') <> '' THEN
      SELECT COALESCE(on_hand, 0) INTO v_lot_stock
...
BEGIN
  ...
  LOOP
    ...
    IF COALESCE(v_item.uses_shared_oil_stock, false) THEN
      ...
      SELECT COALESCE(on_hand, 0) INTO v_source_stock
      FROM inventory_oil_stock_pools
      WHERE factory_id = p_factory_id AND warehouse_id = v_document.source_warehouse_id
      FOR UPDATE;
    ELSE
    ...
    IF COALESCE(NULLIF(trim(v_line.lot_no), ''), '') <> '' THEN
      SELECT COALESCE(on_hand, 0) INTO v_source_lot_stock
      FROM inventory_lot_balances
      WHERE factory_id = p_factory_id AND warehouse_id = v_document.source_warehouse_id
        AND item_id = v_line.item_id AND lot_no = v_line.lot_no
        AND (expiry_date IS NOT DISTINCT FROM v_line.expiry_date)
