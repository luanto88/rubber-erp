-- Chèn guard "ca sản xuất đã khóa" vào 2 RPC ghi/xóa lot_transactions hiện có:
--   - delete_lot_transaction: đổi chữ ký (thêm p_actor_id uuid) — BẮT BUỘC DROP trước vì thêm
--     tham số tạo ra chữ ký khác, CREATE OR REPLACE sẽ chỉ tạo overload mới song song bản cũ
--     (landmine đã từng gặp trong repo — xem lịch sử "function is not unique").
--   - submit_confirm_draft_batch: giữ nguyên chữ ký, chỉ CREATE OR REPLACE thêm guard trong loop.
-- Xem .claude/rules/06-module-production.md mục "Khóa ca sản xuất".
--
-- QUAN TRỌNG: sau khi chạy migration này, mọi client gọi delete_lot_transaction(uuid) (1 tham
-- số) sẽ lỗi "function does not exist" — phải deploy code JS cập nhật (deleteLotTransaction()
-- trong product/actions.ts đổi sang gọi 2 tham số) NGAY SAU khi chạy migration này.

DROP FUNCTION IF EXISTS delete_lot_transaction(uuid);

CREATE FUNCTION delete_lot_transaction(p_transaction_id uuid, p_actor_id uuid)
...
DECLARE
  ...
  v_actor_role text;
BEGIN
  SELECT lt.lot_id, lt.ngan_id, lt.ngay_nhap, lt.ca, l.factory_id
    INTO v_lot_id, v_ngan_id, v_ngay_nhap, v_ca, v_factory_id
  ...
  -- Guard mới: chặn xóa nếu (ngày, ca) của giao dịch đã được duyệt & khóa, trừ admin.
  IF public.product_shift_is_locked(v_factory_id, v_ngay_nhap, v_ca) THEN
    SELECT role INTO v_actor_role FROM profiles WHERE id = p_actor_id;
    IF v_actor_role IS DISTINCT FROM 'admin' THEN
      RAISE EXCEPTION 'Ca sản xuất % - Ca % đã được duyệt & khóa, không thể xóa giao dịch.', v_ngay_nhap, v_ca;
    END IF;
  END IF;
  ...
  -- Qualify lot_transactions.lot_id (alias lt2) — "lot_id" trần bị ambiguous với biến OUT
  -- "lot_id" của RETURNS TABLE ở trên (landmine đã fix ở 20260714_fix_delete_lot_transaction_ambiguous_column.sql).
...
GRANT EXECUTE ON FUNCTION delete_lot_transaction(uuid, uuid) TO authenticated, service_role;

-- submit_confirm_draft_batch giữ nguyên chữ ký — chỉ thêm guard trong vòng lặp, không cần DROP.
...
DECLARE
  ...
  v_touched_lots     UUID[] := '{}';
  v_dup_count        INTEGER;
  v_actor_role       TEXT; -- MỚI: role của p_user_id, tra 1 lần trước loop để check khóa ca
BEGIN
  ...
  -- MỚI: tra role 1 lần cho cả batch (không đổi giữa các dòng trong cùng batch).
  SELECT role INTO v_actor_role FROM profiles WHERE id = p_user_id;
  ...
  LOOP
    -- MỚI: chặn gửi nháp nếu (ngày, ca) của chính nháp đó đã được duyệt & khóa, trừ admin.
    IF public.product_shift_is_locked(v_draft.factory_id, v_draft.ngay_sx, v_draft.ca)
       AND v_actor_role IS DISTINCT FROM 'admin' THEN
      RAISE EXCEPTION 'Ca sản xuất % - Ca % của lô % đã được duyệt & khóa, không thể gửi nháp này.',
        v_draft.ngay_sx, v_draft.ca, v_draft.ma_lo;
    END IF;
