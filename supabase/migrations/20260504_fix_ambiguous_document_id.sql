-- ============================================================
-- Fix: column reference "document_id" is ambiguous
-- RETURNS TABLE declares an OUT param named document_id which
-- conflicts with inventory_document_lines.document_id inside
-- WHERE clauses. Fix: add table alias idl for all queries on
-- inventory_document_lines inside the 3 posting functions.
-- ============================================================
...
BEGIN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM inventory_document_lines idl
    WHERE idl.document_id = p_document_id AND idl.factory_id = p_factory_id
  ) THEN
    RAISE EXCEPTION 'Phiếu nhập % chưa có dòng vật tư.', v_document.document_code;
  END IF;

  FOR v_line IN
    SELECT * FROM inventory_document_lines idl
    WHERE idl.document_id = p_document_id AND idl.factory_id = p_factory_id ORDER BY idl.created_at, idl.id
  LOOP
