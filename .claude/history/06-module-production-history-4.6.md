# Lịch sử phát triển module Sản xuất — phần "4.6 Dự đoán số lô + Quét QR xác nhận"
(`.claude/rules/06-module-production.md`)

> Tách ra ngày 2026-08-22 để giảm dung lượng ngữ cảnh nạp mỗi phiên. Đây là nhật ký chi tiết
> từng phiên code/fix (redesign nhãn in, luồng quét QR "trạm quét"/"Lưu tạm", race condition,
> phiếu báo thành phẩm...) của mục 4.6 trong rule file gốc. Không tự nạp vào context — chỉ đọc
> khi cần tra lại lý do/diễn biến 1 quyết định cụ thể. Quy tắc hiện hành nằm ở
> `.claude/rules/06-module-production.md`.
