# Lịch sử phát triển module KPI & 5S (`.claude/rules/27-kpi-module.md`)

> File này lưu **toàn bộ nhật ký chi tiết theo từng phiên làm việc** (quyết định thiết kế,
> bug đã fix, kế hoạch, checklist test tay...) của module Quản lý công việc & Đánh giá KPI.
> Đây là bản sao nguyên vẹn phần lịch sử đã tách ra khỏi `.claude/rules/27-kpi-module.md`
> ngày 2026-08-22 để giảm dung lượng ngữ cảnh nạp mỗi phiên (rule file gốc đã vượt 5.500
> dòng, phần lớn là nhật ký này).
>
> **File này KHÔNG được tự động nạp vào ngữ cảnh mỗi phiên** — chỉ đọc khi cần tra cứu lại
> lý do/diễn biến của một quyết định cụ thể. Quy tắc/trạng thái HIỆN HÀNH nằm ở
