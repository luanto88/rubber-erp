...
```typescript
// documentQrPath trong receipts / issues / transfers dùng documentId để tránh lỗi encode ký tự "/" trong mã phiếu
const documentQrPath = draft.documentId
  ? `/dashboard/inventory/print?type=import&documentId=${encodeURIComponent(draft.documentId)}`
  : null;
// type = "import" | "export" | "transfer"
```

`qr_value` lưu trong DB (dùng cho QR trên bản in A4 / quét ngoài) vẫn dùng `code`:
...
<div className="flex items-start justify-between gap-4 border-b ...">
  ...
  <div className="flex flex-col items-end gap-3">
    {/* CỤM NÚT HÀNH ĐỘNG (ACTION BAR) */}
    <div className="flex flex-wrap justify-end gap-2">
      {/* Trạng thái draft: [Lưu nháp] [Ghi sổ] */}
      {/* Trạng thái posted: [In phiếu] [Hủy phiếu (nếu có quyền)] */}
      {/* Trạng thái cancelled: [In phiếu] */}
    </div>
...
- **QR in-app (`documentQrPath`) dùng `?documentId=<uuid>`** — tránh lỗi encode dấu `/` trong mã phiếu; print page hỗ trợ cả `documentId` lẫn `code`
- **`qr_value` lưu trong DB dùng `?code=<mã phiếu>`** — dùng cho QR in ra A4 hoặc quét từ bên ngoài
- Cả 2 đều trỏ về `/dashboard/inventory/print?type=...` — read-only, không trỏ về CRUD form
- QR trên bản in A4 self-link qua `qr_value` (không trỏ về CRUD)
...
// Dùng category_name trực tiếp làm tiền tố (không hardcode "Hóa chất" / "Vật tư")
// Ví dụ: category "VTHC" → "VTHC 1", category "Hóa chất" → "Hóa chất 1"
// Fallback khi item null hoặc chưa có category_name → "Vật tư N"
// Đánh số riêng theo từng giá trị category_name
```
...
- Người dùng có thể lọc thêm theo **Phân loại vật tư** (dropdown) để thu hẹp danh sách — dropdown lấy từ `inventory_item_categories`
...
### Hiển thị tồn kho trên card chọn vật tư

Mỗi card vật tư trong phần chọn phải hiển thị:

- **Tổng tồn**: tổng `on_hand` của vật tư đó trên **tất cả kho** từ `inventory_stock_balances`
- **Breakdown từng kho**: chỉ hiện khi vật tư có tồn > 0 ở từ 2 kho trở lên; format `KA: X đơn vị | KB: Y đơn vị`

Logic tính toán (trong map callback của item card):
...
- Chứa danh sách UUID của các kho nơi vật tư được quản lý
- Dùng để filter danh sách vật tư khi người dùng chọn kho trên form
- Điền qua checkbox đa chọn trong Settings → Vật tư
- Một vật tư ở nhiều kho → điền tất cả warehouse IDs; tồn riêng từng kho được theo dõi qua `inventory_stock_balances` (UNIQUE `factory_id, warehouse_id, item_id`)
...
- Với các màn Nhập / Xuất / Chuyển kho:
  - **Cụm nút hành động (Action Bar)**: Nằm trong document info header (góc trên phải), cạnh QR. Không có nút riêng trong Shell header.
  - Trạng thái draft/mới: hiện `[Lưu nháp / Sửa phiếu]` + `[Ghi sổ]`; trạng thái posted/cancelled: ẩn cả 2.
  - "Lưu nháp" (chưa có documentId) / "Sửa phiếu" (đã có documentId): `disabled` khi `!canSave || saving || posting || loading`. `canSave` = kho đã chọn + ít nhất 1 dòng (`!!warehouseId && lines.length > 0`; transfers: cả 2 kho).
  - "Ghi sổ": `disabled` khi `saving || posting || loading`. Nếu chưa có documentId, hàm ghi sổ sẽ tự gọi lưu nháp trước.
  - **Không được ẩn nút** khi form chưa hợp lệ, chỉ dùng `disabled`.
  - Nút "Hủy phiếu" (viền đỏ/chữ đỏ) chỉ xuất hiện khi phiếu ở trạng thái `posted` và user có quyền `inventory.cancel`.
...
- Trigger / function backend được phép dùng để chặn phát sinh xuất vượt tồn
  - Khi viết câu lệnh `JOIN` trong các Stored Procedures (`inventory_post_export_document`, `inventory_post_import_document`, `inventory_post_transfer_document`), bắt buộc phải dùng bí danh (table alias) cho cột `document_id` (ví dụ: `doc.document_id` hoặc `lines.document_id`) để tránh lỗi `column reference "document_id" is ambiguous`.
