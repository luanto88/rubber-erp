...
- **Chưa test tay** — cần: quét QR nhập 1 kiện cho ngăn `Chờ sản xuất` (cả trường hợp <100% và trường hợp 1 lần gửi đã đẩy thẳng lên ≥100%) → xác nhận card ngăn chuyển `Đang sản xuất` ngay; sửa/xóa 1 dòng trong "Lịch sử ca" đổi/xóa hết sản lượng của 1 ngăn → xác nhận trạng thái đồng bộ đúng theo cả 2 chiều; xác nhận luồng nhập tay (`/dashboard/product`) không đổi hành vi.
