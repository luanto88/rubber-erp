...
- **Toàn bộ 6 mục vừa code ở trên (1/2/3/5/6/8) chưa test tay** — xem checklist ngay trên.
- **Test tay nhãn in phiên 5** (xem mục phía trên) — ưu tiên cao vì đụng trực tiếp tới file in thực tế đưa xuống xưởng.
- Migration `20260712_lot_transactions_kien_fields.sql` **chưa chạy** trên Supabase — cần chạy thủ công trước khi cột `boc/pallet/chi_thi` của `lot_transactions` tồn tại.
