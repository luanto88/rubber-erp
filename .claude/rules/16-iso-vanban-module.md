...
## C?p nh?t 2026-06-03 (b? sung) � PDF deploy, footer fallback, suy ra b�?c k?

### PDF sau deploy

- `generate-pdf/route.ts` ph?i �u ti�n d�ng asset local c?a `pdfjs-dist` trong `node_modules` cho `cMap` v� `standard_fonts`, kh�ng ��?c ph? thu?c CDN.
- N?u local asset kh�ng s?n, m?i fallback sang c?u h?nh t?i gi?n.
- M?c ti�u: tr�nh l?i sau deploy khi server kh�ng t?i ��?c asset ngo�i, d?n �?n `metaFillError` v� toast "Kh�ng �?c ��?c text c?a PDF".

### Footer fallback

- N?u `fillMetadataPlaceholders()` tr? `metaResult.error`, route PDF kh�ng ��?c g?i `drawFooterOnAllPages()`.
- Rule n�y �p d?ng cho c? file PDF ch�nh v� file ph? so�t x�t.
- M?c ti�u: tr�nh l?i "header kh�ng ��?c ch�n nh�ng footer l?i �? th�m m?t d?ng m?i to�n t�i li?u".

### Suy ra b�?c k?

- `generate-office/route.ts` v� `generate-pdf/route.ts` ph?i �u ti�n suy ra b�?c k? t? `action`:
  - `gui_xem_xet` -> `soan_thao`
  - `gui_phe_duyet`, `gui_lai_phe_duyet` -> `xem_xet`
  - `phe_duyet` -> `phe_duyet`
- Ch? fallback sang map theo `userId` khi kh�ng c� `action`.
- M?c ti�u: tr�nh ch�n sai tag t�n/ch? k? trong DOCX/XLSX/PDF khi c�ng m?t user ��?c g�n nhi?u vai tr?.
