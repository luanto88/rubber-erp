...
export type DispatchDeliveryPointRow = {
  id?: string
  factory_id?: string
  ma_lo: string
  lat: number
...
export const FACTORY_LAT = 12.58187
export const FACTORY_LNG = 105.497249
...
export const DIEM_GN: DiemGN[] = [
  ...
  { ma_lo: "C16", doi: 5, lat: 12.628052, lng: 105.54629, phien_a: ["A14", "A15", "A16", "A17", "A18", "B14", "B15", "B16", "B17", "B18", "C14", "C15D", "C15T", "C16", "C17", "C18"], phien_b: ["A14", "B15", "B14", "C14"], phien_c: ["A15", "A16", "A17", "A18"], phien_d: ["B18", "B17", "C17", "C18"] },
  ...
  { ma_lo: "D9", doi: 2, lat: 12.62363, lng: 105.513983, phien_a: ["A8", "A9", "A10", "B8", "B9", "B10", "C7", "C8", "C9", "C10", "D6", "D7", "D8", "D10"], phien_b: ["A8", "A9", "A10", "B8"], phien_c: ["B9", "B10", "C9", "C10"], phien_d: ["D6", "D7", "C6", "C7"] },
  ...
  { ma_lo: "G3", doi: 1, lat: 12.610155, lng: 105.48636, phien_a: ["F4", "F5", "G1", "G2", "G3", "G4", "G5", "H1", "H2", "H2D", "H3", "H3D", "H4T", "H4S", "H5", "H6T", "H6S", "I3", "I4", "I5", "J4"], phien_b: ["G1", "G2", "G3", "H1", "H2"], phien_c: ["H3", "H4T", "G4", "F4"], phien_d: ["F5", "G5", "G6"] },
  ...
  { ma_lo: "G9", doi: 3, lat: 12.61007, lng: 105.513951, phien_a: [], phien_b: [], phien_c: [], phien_d: [] },
  ...
  { ma_lo: "I16", doi: 8, lat: 12.600919, lng: 105.54618, phien_a: ["F16", "F17", "F18", "G17", "G18", "H16", "H17", "H18", "I16", "I17", "I18", "J16", "J17", "J18"], phien_b: ["F16", "F17", "F18", "G17"], phien_c: ["G17", "G18", "H18", "H17"], phien_d: ["H17", "H16", "I16", "I17", "I18"] },
  { ma_lo: "J7", doi: 3, lat: 12.596465, lng: 105.5048, phien_a: ["H7", "I6", "I7", "I8", "I9", "J5", "J6T", "J6S", "J7", "J8", "J9", "K5", "K6", "K7", "K8", "K9D", "K9T", "L6T", "L7T", "L7S", "L8T", "L8S"], phien_b: ["H7", "I7", "I8", "I9"], phien_c: ["I6", "J5", "J6T", "J6S", "K5", "K6", "L6T"], phien_d: ["J7", "J8", "J9", "I9"] },
  ...
  { ma_lo: "L2", doi: 4, lat: 12.587526, lng: 105.48175, phien_a: ["I1", "I2", "J1T", "J1D", "J2", "K1", "K2", "K3", "K4", "K5D", "K5T", "L1", "L2", "L3", "L4", "L5D", "M1", "M2", "M3", "N1", "N2"], phien_b: ["I1", "I2", "J1T", "J1D", "J2", "K1"], phien_c: ["K2", "K3", "K4", "K5D", "K5T", "L4", "L5T", "L3"], phien_d: ["L1", "L2", "L3", "M3"] },
  ...
  { ma_lo: "P11", doi: 10, lat: 12.569342, lng: 105.52313, phien_a: ["O9", "O10", "O11", "P10", "P11", "Q10"], phien_b: [], phien_c: [], phien_d: [] },
  ...
  { ma_lo: "U11", doi: 11, lat: 12.545692, lng: 105.5231, phien_a: ["S8", "S9", "S10T", "S10D", "S11", "T9", "T10", "T11", "T12", "U9", "U10", "U11", "U12", "U13", "V9T", "V10T", "V8S", "V9S", "V10S"], phien_b: ["S8", "S9", "T9", "U9"], phien_c: ["U10", "V9T", "V10T", "V8S", "V9S", "V10S"], phien_d: ["S10D", "S10T", "S11", "T10", "T11T", "U11"] },
...
export function normalizeDeliveryPoints(rows?: DispatchDeliveryPointRow[] | null): DiemGN[] {
  const normalized = (rows || [])
    .filter(row => row?.ma_lo)
    .map(row => ({
      ma_lo: row.ma_lo,
      ...
      phien_d: Array.isArray(row.phien_d) ? row.phien_d : [],
    }))

  return normalized.length > 0 ? normalized : DIEM_GN
}
...
export function calcManhattanKm(
  ...
  for (const stop of stops) {
    const point = points.find(item => item.ma_lo === stop)
    if (point) coords.push([point.lat, point.lng])
  }
  if (coords.length === 0) return 0

  let total = 0
  total += Math.abs(factoryLat - coords[0][0]) + Math.abs(factoryLng - coords[0][1])
  for (let i = 1; i < coords.length; i++) {
    total += Math.abs(coords[i][0] - coords[i - 1][0]) + Math.abs(coords[i][1] - coords[i - 1][1])
  }
  total += Math.abs(coords[coords.length - 1][0] - factoryLat) + Math.abs(coords[coords.length - 1][1] - factoryLng)
...
export function buildLoThuHoach(diem_gn: string[], phien: string[], points: DiemGN[] = DIEM_GN): string[] {
  ...
  for (const dgn of diem_gn) {
    ...
    for (const p of phien) {
      ...
      if (Array.isArray(phaseLots)) lots.push(...phaseLots)
    }
  }
  return [...new Set(lots)]
}
