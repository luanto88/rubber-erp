...
export default function OrgChartDraft() {
  ...
  const NodeCard = ({
    ...
    <div
      className={`border-2 border-green-700 bg-white rounded-3xl p-3 text-center shadow-sm z-10 relative ${className}`}
    >
      <div className="font-bold text-gray-800 text-sm">{title}</div>
      {subtitle && (
        <div className="text-xs text-gray-600 font-semibold mt-1 whitespace-pre-line">
          {subtitle}
        </div>
      )}
    </div>
  ...
  const LeafList = ({ items }: { items: string[] }) => (
    <div className="text-left text-xs font-semibold text-gray-700 mt-2 pl-4 border-l-2 border-green-700">
      {items.map((item, idx) => (
        <div
          key={idx}
          className="relative before:content-[''] before:absolute before:-left-4 before:top-2 before:w-3 before:h-0.5 before:bg-green-700"
        >
          {item}
        </div>
      ))}
        ...
        <div className="w-full flex justify-between items-start mt-8 gap-2">
          ...
          <div className="flex justify-between gap-4 relative flex-1 ml-4 border-t-2 border-green-700 pt-8 mt-4">
            ...
            <div className="flex flex-col items-center w-40">
              <NodeCard
                title="Tổ trưởng"
                subtitle="Tổ tiếp nhận đánh đông"
                className="w-full"
              />
              <div className="w-full mt-2">
                <LeafList
