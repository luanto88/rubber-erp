...
export default function InventoryTransfersPage() {
  ...
  const documentQrPath = draft.documentId ? `/dashboard/inventory/print?type=transfer&documentId=${encodeURIComponent(draft.documentId)}` : null
  ...
  const sourceScopedItems = useMemo(() => {
    ...
    return scoped
  }, [draft.sourceWarehouseId, items, selectedCategoryIds])

  const visibleItemCards = useMemo(() => {
    if (draft.selectedItemIds.length === 0) return sourceScopedItems
    return sourceScopedItems.filter((item) => draft.selectedItemIds.includes(item.id))
  }, [draft.selectedItemIds, sourceScopedItems])
  ...
  return (
            ...
            <div className="mb-2 grid gap-3 xl:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_auto]">
              <div>
                <MultiSelectField
                  ...
                  disabled={!draft.sourceWarehouseId}
                />
              </div>
              ...
              <div>
                <MultiSelectField
                  label="Mã vật tư"
                  options={sourceScopedItems.map((item) => ({
                    value: item.id,
                    label: item.code,
                    meta: item.name,
                  }))}
            ...
            {!draft.sourceWarehouseId ? (
              ...
              <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-3">
                {visibleItemCards.map((item) => {
                  ...
                  const warehouseStocks = (draft.sourceWarehouseId
                    ? warehouses.filter((w) => w.id === draft.sourceWarehouseId)
                    : warehouses
                  )
    ...
    {cancelModal ? (
      <ModalShell
        title={<span className="flex items-center gap-2 text-red-600"><Ban size={18} /> Hủy phiếu chuyển kho</span>}
