...
type DocumentPayload = NonNullable<Awaited<ReturnType<typeof fetchInventoryDocumentByReference>>>
...
function DocumentPrintSheet({
  ...
  const qrPath = `/dashboard/inventory/print?type=${documentType}&code=${encodeURIComponent(payload.document.document_code)}`
  const origin = typeof window !== "undefined" ? window.location.origin : ""
  const absoluteQrUrl = `${origin}${qrPath}`

  const warehouseFields = getWarehouseFields(
    documentType,
    sourceWarehouse,
    targetWarehouse,
  ...
  // Người bàn giao/Người nhận: chỉ cho phép fallback sang source_name (tên kho) với Nhập/Chuyển
  // kho — nơi source_name là "Nguồn nhập" (nhà cung cấp/bộ phận giao, một tên PHÙ HỢP để in ở ô
  // này). Với Xuất kho, source_name luôn là tên KHO XUẤT — không phải tên người — nên tuyệt đối
  // không fallback, chỉ in recipient_name (ô "Người nhận hàng" người dùng tự nhập) hoặc để trống.
  const recipientPrintValue =
    documentType === "export"
      ? payload.document.recipient_name || " "
      : payload.document.recipient_name || payload.document.source_name || " "

  const keeperWarehouse = documentType === "import" ? targetWarehouse : sourceWarehouse
  const showApproval = documentType !== "transfer"

  return (
    <div
      className="print-sheet mx-auto max-w-4xl rounded-2xl border border-slate-200 bg-white p-6 shadow-sm"
    ...
    >
      ...
      {showApproval && payload.document.status === "posted" && !payload.document.approved_by_name && (
        <div className="mt-3 flex items-center gap-3 rounded-xl border border-amber-300 bg-amber-50 px-4 py-2 print:hidden">
          <AlertTriangle size={15} className="shrink-0 text-amber-600" />
          <p className="text-xs font-bold text-amber-800">Chưa phê duyệt — cần Ban giám đốc ký xác nhận.</p>
        </div>
      )}
      ...
      {/* Chữ ký */}
      <div className={`print-sig-gap mt-8 grid gap-6 ${showApproval ? "grid-cols-4" : "grid-cols-3"}`}>
        ...
        {showApproval ? (
          <div className="text-center">
            <div className="text-xs font-bold text-slate-700">Người phê duyệt (BGĐ)</div>
            <div className="mt-12 border-t border-slate-300 pt-1.5 text-xs text-slate-500">
              {payload.document.approved_by_name || " "}
            </div>
            {payload.document.approved_by_name && payload.document.approved_at ? (
              <div className="mt-1 flex items-center justify-center gap-1 text-[10px] text-indigo-600">
                <CheckCircle2 size={10} />
                {new Date(payload.document.approved_at).toLocaleDateString("vi-VN")}
              </div>
            ) : null}
          </div>
        ) : null}
...
export default function InventoryPrintPage() {
  ...
  const [batchPayloads, setBatchPayloads] = useState<DocumentPayload[]>([])
  ...
  useEffect(() => {
    const bootstrap = async () => {
      ...
      try {
        ...
        if (batchId) {
          const { data: docsInBatch, error: batchErr } = await supabase
          ...
          if (batchErr || !docsInBatch || docsInBatch.length === 0) {
            setError("Không tìm thấy phiếu nào trong phiên xuất này.")
            return
          }

          const loadedList = await Promise.all(
            docsInBatch.map((row) =>
              fetchInventoryDocumentByReference(factoryId, "export", { documentId: row.id }),
            ),
          )
          const valid = loadedList.filter((v): v is DocumentPayload => Boolean(v))
          if (valid.length === 0) {
            setError("Không tải được dữ liệu các phiếu trong phiên xuất này.")
            return
          }
          setBatchPayloads(valid)
          return
        }
    ...
    void bootstrap()
  }, [batchId, code, documentId, documentType])
  ...
  return (
    <div className="min-h-screen bg-slate-100 p-4 print:bg-white print:p-0">
      ...
      <div className="print-hidden mx-auto mb-4 flex max-w-4xl items-center justify-between gap-3">
        ...
        >
          <Printer size={16} />
          {isBatchMode ? `In ${batchPayloads.length} phiếu (A5 ngang)` : "In phiếu (A5 ngang)"}
        </button>
      </div>

      {loading ? (
        <div className="mx-auto max-w-4xl rounded-2xl border border-slate-200 bg-white py-20 text-center text-slate-400">
          Đang tải dữ liệu phiếu...
        </div>
      ) : error ? (
        <div className="mx-auto max-w-4xl rounded-2xl border border-slate-200 bg-white py-20 text-center text-red-600">
          {error}
        </div>
      ) : isBatchMode ? (
        <div className="space-y-6 print:space-y-0">
          {batchPayloads.map((item, index) => (
            <DocumentPrintSheet
              key={item.document.id}
              documentType="export"
              payload={item}
              sourceWarehouse={item.document.source_warehouse_id ? warehouseMap.get(item.document.source_warehouse_id) ?? null : null}
              targetWarehouse={item.document.target_warehouse_id ? warehouseMap.get(item.document.target_warehouse_id) ?? null : null}
              pageBreakAfter={index < batchPayloads.length - 1}
            />
          ))}
        </div>
      ) : payload ? (
        <DocumentPrintSheet
          documentType={documentType}
          payload={payload}
          sourceWarehouse={payload.document.source_warehouse_id ? warehouseMap.get(payload.document.source_warehouse_id) ?? null : null}
          targetWarehouse={payload.document.target_warehouse_id ? warehouseMap.get(payload.document.target_warehouse_id) ?? null : null}
